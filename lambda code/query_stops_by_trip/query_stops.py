...
def extract_id_param(event, matching_str ="trip_id" ):
...
def query_stops_by_trip_id(table, trip_id):
    return table.query(
        KeyConditionExpression=Key("PK").eq(f"TRIP#{trip_id}")
    ).get("Items", [])


def query_active_stops_by_trip_id(table, trip_id, active_dataset_id):
    

    items = query_stops_by_trip_id(table, trip_id)
...
def lambda_handler(event, context):
    ...
    ##### Parse input
    trip_id = extract_id_param(event)
    ...
    filtered = query_active_stops_by_trip_id(table, trip_id, dataset_id)
